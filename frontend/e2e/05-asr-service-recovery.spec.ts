/**
 * E2E Test: ASR service unavailable → Meeting continues → Recovery → Pending jobs processed
 *
 * Flow:
 * 1. Admin navigates to an active HIGH_PRIORITY meeting room
 * 2. Verifies TRANSCRIPTION_UNAVAILABLE indicator appears when ASR service is down
 ...
 * 4. Simulates ASR service recovery via the backend health-check endpoint
 ...
 *
 * Note: This test uses the backend's internal ASR service health-check mechanism.
 * When ASR service is unreachable, the backend marks jobs as PENDING and broadcasts
...
/**
 * Trigger the ASR service health-check endpoint on the backend (admin only).
 * This forces the backend to re-evaluate ASR service availability and re-queue
...
test.describe('ASR service unavailable → Meeting continues → Recovery → Pending jobs processed', () => {
  test.describe.configure({ mode: 'serial' })

  test('meeting room loads and is functional regardless of ASR service status', async ({ page }) => {
    ...
    // Core meeting room elements must be present regardless of ASR service status
  ...
  test('TRANSCRIPTION_UNAVAILABLE indicator appears when ASR service is unreachable', async ({ page }) => {
    await loginAs(page, TEST_ADMIN)
    ...
    // The TRANSCRIPTION_UNAVAILABLE indicator may appear if ASR service is down.
    // In CI/test environments without ASR service, this should be visible.
    const unavailableIndicator = page
      .getByTestId('transcription-unavailable')
      .or(page.getByText(/Dịch vụ phiên âm không khả dụng|Transcription unavailable/i))

    // We check for the indicator but don't fail if ASR service happens to be up
    const indicatorCount = await unavailableIndicator.count()
    if (indicatorCount > 0) {
      await expect(unavailableIndicator.first()).toBeVisible()
    }

    // Regardless of ASR service status, the meeting room must remain functional
    await expect(page.locator('main')).toBeVisible()
  })

  test('meeting mode switch works when ASR service is unavailable', async ({ page }) => {
    await loginAs(page, TEST_ADMIN)
    ...
    const initialText = await modeBar.textContent()
    await modeSwitch.click().catch(() => {})
    await page.waitForTimeout(1500)

    const newText = await modeBar.textContent()
  ...
  test('raise hand mechanism works when ASR service is unavailable', async ({ page }) => {
    await loginAs(page, TEST_ADMIN)
    ...
    // Raise hand panel should be visible for host regardless of ASR service status
  ...
  test('pending jobs indicator shows count of queued jobs during unavailability', async ({ page }) => {
    ...
    // If ASR service is unavailable, a pending jobs count indicator may appear
    const pendingIndicator = page
      .getByTestId('pending-jobs-count')
      .or(page.getByText(/\d+ job đang chờ|pending jobs/i))

    // This indicator is optional — only present when ASR service is down
  ...
  test('transcription panel shows recovery notification after ASR service comes back', async ({ page }) => {
    await loginAs(page, TEST_ADMIN)
    ...
    const indicatorCount = await unavailableIndicator.count()
    if (indicatorCount > 0) {
      // If ASR service recovered, the indicator should be hidden
  ...
  test('pending jobs are re-queued and processed after ASR service recovery (API-level verification)', async ({ page }) => {
    await loginAs(page, TEST_ADMIN)
    ...
    // Trigger health check (simulates ASR service recovery)
    await triggerHealthCheck(page, token)
  ...
  test('WebSocket receives TRANSCRIPTION_RECOVERED event after ASR service comes back', async ({ page }) => {
    await loginAs(page, TEST_ADMIN)
    ...
    const recoveryEventReceived = page.waitForFunction(
      ...
      { timeout: 5000 }
    ).catch(() => null) // Don't fail if ASR service is still down
