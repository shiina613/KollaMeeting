"""
Kolla ASR Service - FastAPI Application Entry Point
Vietnamese ASR service (PhoWhisper-medium CT2 / Gipformer via sherpa-onnx).
"""
...
app = FastAPI(
    title="Kolla ASR Service",
    version="1.0.0",
    description="Vietnamese ASR microservice (PhoWhisper-medium CT2; optional Gipformer ONNX backend)",
)
...
async def on_startup() -> None:
    ...
    logger.info("=== Kolla ASR Service starting ===")
    ...
    logger.info("  ASR_BACKEND         : %s", settings.ASR_BACKEND)
    logger.info("  PHOWHISPER_MODEL_PATH: %s", settings.PHOWHISPER_MODEL_PATH)
    logger.info("  PHOWHISPER_LANGUAGE : %s", settings.PHOWHISPER_LANGUAGE)
    ...
    logger.info("  ASR_SERVICE_PORT     : %d", settings.ASR_SERVICE_PORT)
...
async def on_shutdown() -> None:
    ...
    logger.info("=== Kolla ASR Service shutting down ===")
...
if __name__ == "__main__":
    uvicorn.run(
        app,
        host="0.0.0.0",
        port=settings.ASR_SERVICE_PORT,
