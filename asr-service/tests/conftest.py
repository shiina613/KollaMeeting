...
# Pre-load the stdlib 'queue' module BEFORE adding the asr-service root to sys.path.
# This prevents the local sr-service/queue/ package from shadowing the stdlib module,
...
if hasattr(sys, "stdlib_module_names") and "queue" not in sys.modules:
    ...
    _asr_service_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    for _path_entry in sys.path:
        if _path_entry == _asr_service_root or _path_entry == "":
            continue
...
# Ensure the asr-service package root is on sys.path so `from core.vad_chunker import ...` works
_asr_service_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
if _asr_service_root not in sys.path:
    sys.path.insert(0, _asr_service_root)
